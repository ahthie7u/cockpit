...
if USE_COSINE_DECAY_RESTARTS:
    ...
    def lr_schedule(num_epochs):
        factors = cosine_decay_restarts(steps_for_cycle=12, max_epochs=num_epochs)
...
for q in configured_quantities("full"):
    ...
    elif q in [quantities.Trace, quantities.TICDiag]:
        quants.append(
    ...
    elif q == quantities.AlphaOptimized:
        quants.append(q(track_schedule=track_schedule, verbose=True))
    elif q == quantities.MaxEV:
        quants.append(q(track_schedule=track_schedule, verbose=True))
    elif q == quantities.MeanGSNR:
        # skip
        pass
        # quants.append(q(track_schedule=track_schedule, verbose=True))
    else:
